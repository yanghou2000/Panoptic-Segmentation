...
def euc_batch(a, b):
    ...
    result = torch.cdist(a, b, p=2, compute_mode="donot_use_mm_for_euclid_dist")
...
def meanshift_torch(data, seed , bandwidth, max_iter=300):
    ...
    while True:
        ...
        # Yang: replace the operation with optimized function in pytorch
        # num = (weight[:, :, None] * X).sum(dim=1)
        num = torch.matmul(weight, X)
        S_old = S
